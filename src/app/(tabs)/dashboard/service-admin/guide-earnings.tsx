...
export default function GuideEarningsScreen() {
  ...
  const transactions = useMemo(
    () =>
      [...bookings]
        .sort(
          (a, b) =>
            new Date(b.bookedAt || b.bookingDate || 0).getTime() -
            new Date(a.bookedAt || a.bookingDate || 0).getTime()
        )
        .slice(0, 30),
    [bookings]
  );
  ...
  const loading = (guideLoading || bookingsLoading) && !refreshing;
    ...
    <SafeAreaView edges={['top', 'bottom']} className="flex-1 bg-background dark:bg-background-dark">
      <View className="px-6 pt-4 pb-2">
        ...
        <Text className="text-2xl font-bold font-heading text-text dark:text-text-dark">
          {t(TRANSLATION_KEYS.DASHBOARD.ADMIN_CARDS.GUIDE_EARNINGS)}
        </Text>
        <Text className="mt-1 text-sm text-muted dark:text-muted-dark">
          {t(TRANSLATION_KEYS.DASHBOARD.ADMIN_CARDS.GUIDE_EARNINGS_DESC)}
        </Text>
      </View>
      ...
      {loading ? (
        ...
        <View className="px-6">
          <Text className="text-sm text-text dark:text-text-dark">
            {guideError || t(TRANSLATION_KEYS.GUIDE_BOOKING.NO_PROFILE)}
          </Text>
        </View>
      ) : (
        <FlatList
          ...
          ListHeaderComponent={
            <View className="mb-4">
              ...
              <View className="flex-row gap-3 mb-4">
                {card(
                  t(TRANSLATION_KEYS.GUIDE_BOOKING.COMPLETED_TOURS),
                  `${summary.completedTours}`
                )}
                {card(
                  t(TRANSLATION_KEYS.GUIDE_BOOKING.ACTIVE_BOOKINGS),
